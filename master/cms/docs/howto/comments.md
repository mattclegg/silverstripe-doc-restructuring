...
There is a new 'Comments' administration area in the CMS. Comments are seperated into three tables - Approved, Awaiting Moderation, and Spam. The table allow mass deletion and reclassification of comments. By default all comments not detected as spam go into the Approved folder. You can add the line "PageComment::enableModeration();" into the _config.php to enable moderation, which means all new comments go into the Awaiting Moderation folder. Comments awaiting moderation will not be shown on the front end page unless the user is an administrator.
