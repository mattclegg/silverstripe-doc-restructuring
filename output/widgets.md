...
As of 2.2.1 this is a way to add widgets to other pages (by default only the Blog has widgets enabled). In the future
releases we will hopefully make widgets part of SiteTree therefore available on every page. In the mean time you have to
do a couple things to get a Widget to work on a page.

First step is to add a WidgetArea to the Database to store the widget details. Then you have to edit the CMS to add a
